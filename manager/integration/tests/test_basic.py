...
def test_filesystem_trim(client, fs_type):
    ...
    assert snapMap[snap14_origin.name].name == snap14_origin.name
    assert snapMap[snap14_origin.name].removed is False
    assert snapMap[snap14_origin.name].size == snap14_origin.size
    assert snapMap[snap14_origin.name].parent == snap12_origin.name

    assert snapMap[VOLUME_HEAD_NAME].name == VOLUME_HEAD_NAME
    assert int(snapMap[VOLUME_HEAD_NAME].size) >= 100 * Mi
    assert snapMap[VOLUME_HEAD_NAME].parent == snap14_origin.name
    volume_head2 = snapMap[VOLUME_HEAD_NAME]
    ...
    assert snapMap[snap14_origin.name].name == snap14_origin.name
    assert snapMap[snap14_origin.name].removed is False
    assert int(snapMap[snap14_origin.name].size) <= \
           int(snap14_origin.size) - 100 * Mi
    assert snapMap[snap14_origin.name].parent == snap12_origin.name

    assert snapMap[VOLUME_HEAD_NAME].name == VOLUME_HEAD_NAME
    assert int(snapMap[VOLUME_HEAD_NAME].size) <= \
           int(volume_head2.size) - 100 * Mi
    assert snapMap[VOLUME_HEAD_NAME].parent == snap14_origin.name
